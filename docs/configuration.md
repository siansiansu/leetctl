...
leetctl reads `~/.leetcode/leetcode.toml`. The file is created with defaults the first time you run any command, so the easiest way to start is to run leetctl once and then edit the file.
...
| `lang` | `'rust'` | Language of the generated file. `leetctl edit --lang <lang>` overrides this per-call and persists it. |
...
With the above, `leetctl edit 15` produces:
